# Historical coordinator workflow

This manual preserves the 0.2.x coordinator design for existing projects that explicitly retain it. It is not the default Shared Memory 0.3.0 workflow. Use [the current operating guide](PRODUCT-V1.md) for direct folder editing. Existing coordinator projects require an explicit backed-up migration to change workflows; no hidden acceptance or automatic migration is implied.
